...
public class Gamepieces extends Subsystems{
  ...
  public int armPosition(double elevatorPos) {
    return (int)(-1*Math.round(1.2*((180/Math.PI)*(Math.asin(((Constants.CLIMB_HEIGHT*(elevatorPos)/Constants.ELEVATOR_CLIMB)+Constants.ARM_CLIMB_START_HEIGHT-Constants.ARM_ORIGIN)/Constants.ARM_LENGTH))-Constants.ARM_CLIMB_START_ANGLE)*(4096/120)));
    }
