...
public class Path {
	 ...
	 */
	public void initializePath(double maxVel, double maxAccel, double maxVelk, double endVelocity) {
		setCurvatures();
		setDistances();
		setTargetVelocities(maxVel, maxAccel, maxVelk, endVelocity);
	}
	...
	public void setTargetVelocities(double maxVel, double maxAccel, double k) {
		setTargetVelocities(maxVel, maxAccel, k,0);
	}

	public void setTargetVelocities(double maxVel, double maxAccel, double k, double endVelocity) {
		robotPath.get(robotPath.size() - 1).setVelocity(endVelocity);
		...
		robotPath.get(0).setVelocity(1);
		for (int i = 1; i <= robotPath.size() - 1; i++) {
			double distance = Vector.dist(robotPath.get(i - 1), robotPath.get(i));
			double maxReachableVel = Math.sqrt(Math.pow(robotPath.get(i - 1).getVelocity(), 2) + (2 * maxAccel * distance));
			robotPath.get(i).setVelocity(Math.min(maxReachableVel, robotPath.get(i).getVelocity()));
			//System.out.println(robotPath.get(i) + ", v: " + robotPath.get(i).getVelocity());
		}
	}
